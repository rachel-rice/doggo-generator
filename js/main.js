...
fetch(BREEDS_URL)
    ...
    .then(data => {
        const breedsObject = data.message; //Turn the message in to an object
        const breedsArray = Object.keys(breedsObject) //Turn the object in to an array
        for(let i =0; i < breedsArray.length; i++){
            const option = document.createElement('option') //basically creating: <option></option>
            option.value = breedsArray[i] //<option value="breed">
            option.innerText = breedsArray[i] //<option value="breeed">breed</option>
    ...
    const getDoggoImg = url => {
        fetch(url) //going to the API url above
            ...
            .then(data => {
                img.src = data.message //extract message from JSON and attach to attch to img tag as new source
            })
    }
